module ResourceThis # :nodoc:
  ...
  module ClassMethods
    def resource_this(options = {})
      options.assert_valid_keys(:class_name, :will_paginate, :finder_options, :nested, :path_prefix, :per_page)
      ...
      if will_paginate_index
        module_eval <<-"end_eval", __FILE__, __LINE__
          def load_#{plural_name}
            @#{plural_name} = finder_base.paginate(finder_options.merge(:page => params[:page], :per_page => #{per_page}))
          end
        end_eval
      else
        module_eval <<-"end_eval", __FILE__, __LINE__
